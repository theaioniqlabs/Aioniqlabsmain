...
// Default card data - can be overridden via props
...
const BentoCardGrid: React.FC<{
  ...
  <div
    className="bento-section grid select-none relative w-full"
    style={{ 
...
// Card Accordion Component - Embedded accordion for Magic Bento cards
...
const CardAccordionItem: React.FC<CardAccordionItemProps> = ({ item, isActive, onMouseEnter, cardHeight = 200 }) => {
  return (
    <div
      className={`
...
const CardAccordion: React.FC<CardAccordionProps> = ({ items, activeIndex, onItemHover, cardHeight = 200 }) => {
  return (
    <div 
      className="flex flex-row items-center justify-center gap-4 overflow-x-auto overflow-y-hidden w-full h-full absolute inset-0"
      style={{
...
const MagicBento: React.FC<BentoProps> = ({
  ...
  // Accordion state management - track active index per card
  const [accordionStates, setAccordionStates] = useState<Record<string | number, number>>({})
  
  const handleAccordionHover = useCallback((cardId: string | number, index: number) => {
    ...
    <>
      <style dangerouslySetInnerHTML={{__html: `
          .bento-section {
            ...
            --background-dark: var(--color-background-primary);
          ...
          .card-responsive {
            ...
            margin: 0 auto;
            gap: var(--spacing-stack-gap-sm);
            padding: var(--spacing-stack-gap-sm);
          }
          ...
          .card--border-glow::after {
            ...
            background: radial-gradient(var(--glow-radius) circle at var(--glow-x) var(--glow-y),
                ...
                transparent 60%);
            border-radius: var(--radii-card-default);
      ...
      <BentoCardGrid gridRef={gridRef}>
        <div className="card-responsive grid gap-2">
          {cards.map((card, index) => {
            const baseClassName = `card flex flex-col justify-between relative aspect-[4/3] min-h-[200px] w-full max-w-full border border-solid font-light overflow-hidden transition-all duration-300 ease-in-out hover:-translate-y-0.5 hover:shadow-[0_8px_25px_rgba(0,0,0,0.15)] ${
              enableBorderGlow ? 'card--border-glow' : ''
            }`

            const cardStyle = {
              backgroundColor: card.color || 'var(--color-background-primary)',
            ...
            if (enableStars) {
              return (
                <ParticleCard
                  ...
                  style={{
                    ...cardStyle,
                    padding: card.showAccordion ? '0' : cardStyle.padding, // Remove padding for accordion
                  }}
                  disableAnimations={shouldDisableAnimations}
                ...
                >
                  {card.showAccordion && card.accordionItems ? (
                    <CardAccordion
                      items={card.accordionItems}
                      activeIndex={accordionStates[card.id || index] ?? 4}
                      onItemHover={(itemIndex) => handleAccordionHover(card.id || index, itemIndex)}
                      cardHeight={200}
                    />
                  ) : (
                    <>
                      <div className="card__header flex justify-between gap-3 relative text-white">
                        <span 
                          className="card__label font-body"
                          style={{
                      ...
                      <div className="card__content flex flex-col relative text-white">
                        <h3 
                          className={`card__title font-heading m-0 mb-1 ${textAutoHide ? 'text-clamp-1' : ''}`}
                          style={{
                        ...
                        <p
                          className={`card__description font-body opacity-90 ${textAutoHide ? 'text-clamp-2' : ''}`}
                          style={{
                            fontSize: 'var(--typography-body-subtle-size-desktop)',
                            lineHeight: 'var(--typography-body-subtle-line-height-desktop)',
