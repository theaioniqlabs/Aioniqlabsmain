...
export const TypewriterTestimonial: React.FC<TypewriterTestimonialProps> = ({ testimonials }) => {
  ...
  return (
    <div className="flex justify-center items-center gap-2 sm:gap-3 md:gap-4 flex-wrap p-4 sm:p-6 w-full h-full relative overflow-visible" style={{ minHeight: 'var(--spacing-image-height-card)' }}>
          ...
          <AnimatePresence>
            {hoveredIndex === index && (
              <motion.div
                ...
                className="absolute bottom-full mb-2 sm:mb-3 md:mb-4 bg-white text-black text-xs sm:text-sm px-3 py-2 sm:px-4 sm:py-3 rounded-lg shadow-2xl sm:max-w-xs sm:w-48 md:w-56 left-1/2 -translate-x-1/2 z-50"
                style={{
                  maxWidth: 'var(--spacing-image-width-tooltip-default)',
                  width: 'var(--spacing-image-width-tooltip-small)',
                }}
              >
                ...
                <p 
                  className="text-right text-gray-500 sm:text-xs md:text-sm"
                  style={{
                    fontSize: 'var(--typography-caption-extra-small-size)',
                    lineHeight: 'var(--typography-caption-extra-small-line-height)',
                  }}
                >
